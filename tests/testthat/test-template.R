...
#    `functionName` is a function's name in your module (e.g., `mpbClimateEvent1`).
# 2. Copy this file to the tests folder (i.e., `/home/achubaty/Documents/GitHub/MPB/SpaDES/modules/mpbClimate/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("mpbClimate")
  path <- list(modulePath = "/home/achubaty/Documents/GitHub/MPB/SpaDES/modules",
               outputPath = file.path(tempdir(), "outputs"))
  parameters <- list(
    #.progress = list(type = "graphical", interval = 1),
    .globals = list(verbose = FALSE),
    mpbClimate = list(.saveInitialTime = NA)
  )
  ...
  if (exists("mpbClimateEvent1", envir = .GlobalEnv)) {
    simOutput <- mpbClimateEvent1(mySim)
  } else {
    simOutput <- mySim$mpbClimateEvent1(mySim)
  }
  ...
  if (exists("mpbClimateEvent2", envir = .GlobalEnv)) {
    simOutput <- mpbClimateEvent2(mySim)
  } else {
    simOutput <- mySim$mpbClimateEvent2(mySim)
  }
